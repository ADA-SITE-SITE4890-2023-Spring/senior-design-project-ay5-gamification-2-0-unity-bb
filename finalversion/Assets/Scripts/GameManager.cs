...
public class GameManager : MonoBehaviour
{
    ...
    public List<Users> selectedUsersList = new List<Users>();
    public List<Transform> userButtons;
    public List<Transform> selectedUserButtons;
    public List<Dictionary<string, Users>> userDictionary = new List<Dictionary<string, Users>>();
    ...
    public void SetUserButtons(Transform button)
    {
        int i = userButtons.IndexOf(button);
        selectedUserButtons[i].gameObject.SetActive(true);
        selectedUserButtons[i].GetComponentInChildren<Text>().text = userButtons[i].GetComponentInChildren<Text>().text;
        selectedUsersList.Add(userDictionary[i][userButtons[i].GetComponentInChildren<Text>().text]);
        userButtons[i].gameObject.SetActive(false);
        selectedUsersList.Add(usersList[i]);

    }


    public void SendMessagestoApi()
    {
        List<Users> _SelectedUserList = new List<Users>();

        APIManager.Instance.CreateMessage(subjectText.text, bodyText.text, selectedUsersList, authContext.access_token);
    }
}