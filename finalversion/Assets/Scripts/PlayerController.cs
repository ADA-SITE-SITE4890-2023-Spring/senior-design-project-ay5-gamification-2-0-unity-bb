...
public class PlayerController : MonoBehaviour
{
    ...
    private async void OnCollisionEnter(Collision other)
    {
        if (other.gameObject.tag == "Portal")
        {
            ...
            GameManager.Instance.usersList = await APIManager.Instance.GetUsers(currentRoom, GameManager.Instance.authContext.access_token);
            for (int i = 0; i < GameManager.Instance.usersList.Count; i++)
            {
                GameManager.Instance.userButtons[i].gameObject.SetActive(true);
                GameManager.Instance.userButtons[i].transform.GetComponentInChildren<Text>().text = GameManager.Instance.usersList[i].userName;
                Dictionary<string, Users> newUser = new Dictionary<string, Users>();
                newUser.Add(GameManager.Instance.usersList[i].userName, GameManager.Instance.usersList[i]);
                GameManager.Instance.userDictionary.Add(newUser);
