...
public class APIManager : MonoBehaviour
{
    ...
    // Get course's quizzes : API - Assessments
    ...
    // Get course's quiz questions : API - Assessments
    ...
    // Get course's quiz questions : API - User
    ...
    // Get course's quiz questions : API - Content
    public async Task<Content> GetRootContentAsync(string courseId, string access_token)
    {
        Content courseContent = new Content();
    ...
    public async void CreateMessage(string subject, string messageBody, List<Users> recipients, string access_token)
    {
      ...
      ""body"": ""{messageBody}"",
      ""toUsers"": [
        {string.Join(",", recipientIds.Select(x => $"\"{x}\""))}
      ],
      ""ccUsers"": [],
      ""bccUsers"": [],
    ...
    public async Task<List<Users>> GetUsers(string courseId, string access_token)
    {
        List<Users> userList = new List<Users>();
        using var www5 = UnityWebRequest.Get($"{BASE_URL}//learn/api/public/v1/courses/courseId:" + courseId + "/users");
        www5.SetRequestHeader("Authorization", $"Bearer {access_token}");
        www5.SetRequestHeader("Content-Type", "application/json");

        var operation = www5.SendWebRequest();
        ...
        {
            Debug.Log($"Failed: {www5.error}");
        }

        var jsonResponse = www5.downloadHandler.text;
        try
        {
            UsersList users = JsonUtility.FromJson<UsersList>(jsonResponse);
            foreach (Users user in users.results)
            {
                var _user = await GetUserInfo(user.userId, access_token);
                userList.Add(_user);
            }
        }
        catch (Exception ex)
    ...
    public async Task<Users> GetUserInfo(string userId, string access_token)
    {
        Users user = new Users();
        using var www5 = UnityWebRequest.Get($"{BASE_URL}//learn/api/public/v1/users/" + userId);
        www5.SetRequestHeader("Authorization", $"Bearer {access_token}");
        www5.SetRequestHeader("Content-Type", "application/json");
        ...
        try
        {
            user = JsonUtility.FromJson<Users>(jsonResponse);
        }
        catch (Exception ex)
        {
            Debug.Log(ex.Message);
